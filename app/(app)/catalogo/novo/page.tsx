...
export default function NovoCatalogoPage() {
  const router=useRouter(); const {barbershop,insertRecord,updateRecord,deleteRecord}=useAppData()
  ...
  async function save(){if(!form.name.trim()){setStatus('Informe o nome do item.');return} if(imageFile && !isCatalogImageFile(imageFile)){setStatus('Escolha uma imagem JPG, PNG, WEBP ou SVG de até 2 MB.');return} const r=await insertRecord('catalog_items',{barbershop_id:barbershop.id,type:form.type,category:form.category.trim()||null,name:form.name.trim(),price:number(form.price),cost:number(form.cost),commission:number(form.commission),duration_min:form.type==='servico'?number(form.duration):null,stock:form.type==='produto'?number(form.stock):null,min_stock:form.type==='produto'?number(form.minStock):null,active:true,image_url:null});if(r.error){setStatus(r.error);return} const itemId=(r.data as {id?:string})?.id; if(imageFile && itemId){const supabase=createBrowserSupabaseClient(); const path=getCatalogImagePath(barbershop.id,itemId,imageFile); const upload=await supabase.storage.from('barbershop-assets').upload(path,imageFile,{contentType:imageFile.type,upsert:false}); if(upload.error){await deleteRecord('catalog_items',itemId);setStatus(upload.error.message);return} const imageUrl=supabase.storage.from('barbershop-assets').getPublicUrl(path).data.publicUrl; const updated=await updateRecord('catalog_items',itemId,{image_url:imageUrl}); if(updated.error){setStatus(updated.error);return}} router.push('/catalogo')}
  ...
  </div><Field label="Imagem (opcional)"><Input type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" onChange={e=>setImageFile(e.target.files?.[0] ?? null)}/><p className="text-xs text-muted-foreground">JPG, PNG, WEBP ou SVG · até 2 MB</p></Field>{status?<p className="mt-4 text-sm text-destructive">{status}</p>:null}<Button variant="gold" className="mt-5 w-full" onClick={save}><Save className="size-4"/>Salvar item</Button></Card></div>
}
