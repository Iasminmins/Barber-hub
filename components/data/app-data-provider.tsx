...
export function AppDataProvider({ children }: { children: React.ReactNode }) {
  ...
  const load = React.useCallback(async () => {
    ...
    setValue({
      ...
      subscriptions: subscriptions.map((r:any)=>({ id:safeText(r.id), barbershopId:safeText(r.barbershop_id), planId:safeText(r.plan_id), planName:safeText(r.plan_name), clientId:safeText(r.client_id), clientName:safeText(r.client_name), employeeId:safeText(r.employee_id), employeeName:safeText(r.employee_name), price:num(r.price), startDate:safeText(r.start_date), dueDate:safeText(r.due_date), status:safeText(r.status), creditsUsed:r.credits_used??undefined, creditsTotal:r.credits_total??undefined })),
  ...
  React.useEffect(() => {
    ...
    const channel = supabase
      .channel(`barberhub-appointments-${shopId}`)
      .on(
        'postgres_changes',
        {
          event: 'INSERT',
        ...
        (payload) => {
          window.dispatchEvent(new CustomEvent('barberhub:new-appointment', { detail: payload.new }))
          void load()
        },
      )
      .subscribe()
