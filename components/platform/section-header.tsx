...
export function SectionHeader({ title, description, insights, icon: Icon = Sparkles, className }: SectionHeaderProps) {
  return (
    <div className={cn('overflow-hidden rounded-xl border border-border/60 bg-card pf-card-lift', className)}>
        ...
        <div className="grid grid-cols-2 divide-x divide-border/70 border-t border-border/70 sm:grid-cols-4">
          {insights.map((insight) => (
            <div key={insight.label} className="px-4 py-3">
              <p className="truncate text-[11px] uppercase tracking-wide text-muted-foreground">{insight.label}</p>
              <p className={cn('mt-1 truncate text-base font-medium tabular-nums sm:text-lg', toneClass[insight.tone ?? 'default'])}>{insight.value}</p>
            </div>
          ))}
        </div>
      ) : null}
