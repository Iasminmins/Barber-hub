drop function if exists public.get_public_available_slots(text, uuid, date);
drop function if exists public.create_public_appointment(text, uuid, date, time, text, text, text);
...
as $$
  select jsonb_build_object(
    'barbershop', jsonb_build_object('name', b.name, 'slug', b.slug, 'city', coalesce(b.city, ''), 'color', b.color),
    'services', coalesce((
      select jsonb_agg(jsonb_build_object('id', c.id, 'name', c.name, 'category', coalesce(c.category, ''), 'price', c.price, 'durationMin', coalesce(c.duration_min, 40)) order by c.name)
    ...
    'employees', coalesce((
      ...
      where e.barbershop_id = b.id and e.active = true
...
begin
  ...
  if target_employee.id is null then raise exception 'Este horário acabou de ficar indisponível para o profissional escolhido.'; end if;
