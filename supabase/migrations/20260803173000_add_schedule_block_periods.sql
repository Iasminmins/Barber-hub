...
alter table public.schedule_blocks
  drop constraint if exists schedule_blocks_employee_id_date_key;

alter table public.schedule_blocks
  add constraint schedule_blocks_period_shape_check check (
    (start_time is null and end_time is null)
    or (start_time is not null and end_time is not null and start_time < end_time)
  );

create index if not exists idx_schedule_blocks_employee_date_period
on public.schedule_blocks (employee_id, date, start_time, end_time);

create or replace function public.prevent_overlapping_schedule_blocks()
returns trigger
language plpgsql
...
begin
  if exists (
    ...
    where sb.employee_id = new.employee_id
      and sb.date = new.date
      and sb.id <> new.id
      and (
        sb.start_time is null
        or new.start_time is null
        or (new.start_time < sb.end_time and new.end_time > sb.start_time)
      )
  ) then
    raise exception 'Já existe um bloqueio neste período.';
  end if;
  return new;
end;
$$;

revoke all on function public.prevent_overlapping_schedule_blocks() from public;

drop trigger if exists prevent_overlapping_schedule_blocks on public.schedule_blocks;
create trigger prevent_overlapping_schedule_blocks
before insert or update of employee_id, date, start_time, end_time on public.schedule_blocks
for each row execute function public.prevent_overlapping_schedule_blocks();

create or replace function public.prevent_appointment_on_blocked_day()
returns trigger
language plpgsql
set search_path = public
as $$
begin
  -- Keep existing appointments editable when their professional, date and time did not change.
  if tg_op = 'UPDATE'
    and new.employee_id is not distinct from old.employee_id
    and new.date is not distinct from old.date
    and new.start is not distinct from old.start
  ...
  if exists (
    ...
    where sb.employee_id = new.employee_id
      ...
      and (
        sb.start_time is null
        or (
          new.start < sb.end_time
          and (new.start + make_interval(mins => coalesce(new.duration_min, 40))) > sb.start_time
        )
      )
  ) then
    raise exception 'A agenda deste barbeiro está bloqueada neste horário.';
  end if;
  return new;
end;
$$;
...
before insert or update of employee_id, date, start, duration_min on public.appointments
...
create or replace function public.get_public_available_slots(
  p_slug text, p_service_id uuid, p_date date, p_employee_id uuid default null
)
...
declare
  ...
  result jsonb;
begin
  if p_date < current_date or p_date > current_date + 60 then return '[]'::jsonb; end if;
  select * into target_shop from public.barbershops where lower(trim(slug)) = lower(trim(p_slug));
  if target_shop.id is null then return '[]'::jsonb; end if;

  select coalesce(duration_min, 40) into service_duration
  from public.catalog_items
  where id = p_service_id and barbershop_id = target_shop.id and type = 'servico' and active = true;
  if service_duration is null then return '[]'::jsonb; end if;

  weekday_key := (array['domingo','segunda','terca','quarta','quinta','sexta','sabado'])[extract(dow from p_date)::integer + 1];
  day_config := coalesce(target_shop.agenda_settings->'businessHours'->weekday_key,
    jsonb_build_object('closed', weekday_key = 'domingo', 'start', '09:00', 'end', '19:30'));
  if coalesce((day_config->>'closed')::boolean, false) then return '[]'::jsonb; end if;
  opens_at := coalesce((day_config->>'start')::time, '09:00'::time);
  closes_at := coalesce((day_config->>'end')::time, '19:30'::time);

  select coalesce(jsonb_agg(to_char(slot_value, 'HH24:MI') order by slot_value), '[]'::jsonb) into result
  from generate_series(p_date + opens_at, p_date + closes_at - make_interval(mins => service_duration), interval '30 minutes') slot_value
  where slot_value > now()
    and exists (
      select 1 from public.employees e
      where e.barbershop_id = target_shop.id and e.active = true
        ...
        and not exists (
          ...
          where sb.employee_id = e.id and sb.date = p_date
            and (
              sb.start_time is null
              or (slot_value::time < sb.end_time and (slot_value::time + make_interval(mins => service_duration)) > sb.start_time)
            )
        )
        and not exists (
          select 1 from public.appointments a
          where a.employee_id = e.id and a.date = p_date and a.status <> 'cancelado'
...
begin
  if char_length(clean_name) < 2 then raise exception 'Informe seu nome.'; end if;
  ...
  select * into target_service from public.catalog_items
  where id = p_service_id and barbershop_id = target_shop.id and type = 'servico' and active = true;
  ...
  where e.barbershop_id = target_shop.id and e.active = true
    ...
    and not exists (
      ...
      where sb.employee_id = e.id and sb.date = p_date
        and (
          sb.start_time is null
          or (p_start < sb.end_time and (p_start + make_interval(mins => coalesce(target_service.duration_min, 40))) > sb.start_time)
        )
    )
    and not exists (
      select 1 from public.appointments a
  ...
  select * into target_client from public.clients c
  where c.barbershop_id = target_shop.id and regexp_replace(coalesce(c.phone, ''), '\D', '', 'g') = clean_phone
  order by c.created_at limit 1;
  if target_client.id is null then
    insert into public.clients (barbershop_id, name, phone) values (target_shop.id, clean_name, clean_phone) returning * into target_client;
  else
    update public.clients set name = clean_name, phone = clean_phone where id = target_client.id returning * into target_client;
  end if;
  ...
  insert into public.appointments (
    ...
    target_employee.name, target_service.name, p_date, p_start, coalesce(target_service.duration_min, 40),
    'agendado', target_service.price, nullif(trim(coalesce(p_notes, '')), '')
  ) returning id into created_appointment;

  return jsonb_build_object(
    'appointmentId', created_appointment, 'barbershopName', target_shop.name,
    'serviceName', target_service.name, 'employeeName', target_employee.name,
    'date', p_date, 'start', to_char(p_start, 'HH24:MI')
  );
end;
$$;
