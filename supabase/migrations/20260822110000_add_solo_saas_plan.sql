-- Adiciona o plano Solo, voltado para operações com 1 a 2 barbeiros.

alter table public.barbershops
  drop constraint if exists barbershops_plan_check;

alter table public.barbershops
  add constraint barbershops_plan_check
  check (plan in ('solo', 'starter', 'pro', 'premium'));
...
begin
  ...
  safe_plan := case when selected_plan in ('solo', 'starter', 'pro', 'premium') then selected_plan else 'starter' end;
  clean_document := regexp_replace(coalesce(billing_document, ''), '\D', '', 'g');
  if clean_document = '' then
    clean_document := null;
  end if;

  clean_phone := regexp_replace(coalesce(owner_phone, ''), '\D', '', 'g');
  ...
  insert into public.members (barbershop_id, user_id, name, email, phone, role, active)
  values (
...
revoke all on function public.create_barbershop_for_current_user(text, text, text, text, text, text) from public, anon;
grant execute on function public.create_barbershop_for_current_user(text, text, text, text, text, text) to authenticated;
